...
攻击分析.md  ──→  generate_attacks.py  ──→  攻击集_L1.jsonl
                                              │
HarmBench CSV ──→  import_harmbench.py  ──→  harmbench_prompts.jsonl
                                              │
                              ┌───────────────┴───────────────┐
                              ▼                               ▼
              ...
              各数据集结果  elo.json  过敏报告    实时ELO  按需孪生  综合报告
                              ...
                              └───────────┬───────────────────┘
                                   ...
                                   cluster.py                     report.py
                                          │                              │
                                  cluster_report.json    security_tree.json
                                                        security_report.md
```
...
| 导入 | `HarmBench/` CSV | 转换 behavior 为我们的 prompt 格式 | `harmbench_prompts.jsonl` (300条) |
| 评估 | 任意 `.jsonl` prompt 文件 | 目标LLM响应 → Judge评分 → ELO更新 | `{输入名}_结果.jsonl` + `{输入名}_汇总.json` + `elo.json` |
| 编排 | 攻击集 + ELO | 三阶段自适应: 攻击→过敏→评判 | `runner_report.json` + `security_tree.json` + `security_report.md` |
| 聚类 | 评估结果 + `elo.json` | 特征提取 → 肘部法则 → K-Means | `cluster_report.json` + `cluster_matrix.csv` |
...
- 对每种方法，调用API生成5条攻击prompt，每条对应一个有害类别（violence/hate/fraud/privacy/self_harm/illegal）
...
```bash
# Step 1: 准备攻击集
python generate_attacks.py                        # 生成我们自己的攻击集
python import_harmbench.py                        # 导入 HarmBench 数据集

# Step 2: 自适应评估（三阶段一条龙）
python runner.py                                  # 全流程
python runner.py --input harmbench_prompts.jsonl  # 用HarmBench数据
python runner.py --batch-size 20 --max-rounds 3   # 自定义参数

# Step 3: 聚类分析
python cluster.py

# 查看结果
# output/runner_report.json     — 综合报告（ASR + FPR + ELO边界 + 建议）
...
python import_harmbench.py

# Step 2: 全量评估
python evaluate.py --max-samples 20
python evaluate.py --input harmbench_prompts.jsonl
...
# 查看结果
# output/攻击集_L1_汇总.json       — 我们的统计
# output/harmbench_prompts_汇总.json — HarmBench统计
```
...
| 文件 | 格式 | 说明 |
|------|------|------|
| `output/攻击集_L1.jsonl` | JSONL | 我们生成的攻击prompt（含数学题） |
| `output/harmbench_prompts.jsonl` | JSONL | HarmBench 转换后的300条prompt |
| `output/{输入名}_结果.jsonl` | JSONL | 逐条评估结果（依输入文件命名） |
| `output/{输入名}_汇总.json` | JSON | 统计摘要（含ELO + Judge统计） |
| `output/elo.json` | JSON | ELO评分持久化状态（跨数据集累积） |
| `output/runner_report.json` | JSON | 统一评估报告（ASR + FPR + ELO边界 + 修复建议） |
| `output/safe_twins.jsonl` | JSONL | 安全孪生集（runner可自动追加生成） |
| `output/allergy_results.jsonl` | JSONL | 过敏测试逐条结果 |
| `output/allergy_report.json` | JSON | 过敏报告 + 2D安全画像 |
| `output/security_tree.json` | JSON | 树形安全画像（5维分解 + 方法级详情） |
| `output/security_report.md` | Markdown | LLM润色的人类可读安全评估报告 |
