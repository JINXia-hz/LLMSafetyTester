...
1. **R 矩阵是唯一真相**：结果矩阵 `R[method][model] = MatchResult` 是整个评估体系的唯一不可重算观测（花 API 成本换来的）。Elo、预测器、收敛判定全是从 R + 方法特征 X **派生**的缓存，可随时全量重算——缓存丢失无碍。这保证：Elo 不跨模型混淆（每个模型的 Elo 仅由该模型列的 R 回放得到）、多模型天然支持（R 的第二维就是模型）、可重建。
...
R 矩阵是唯一真相——下列产物中，`results.json`（R）是权威存储，`elo_cache.json` / `predictors/` 是派生缓存（可删可重建），`state.json` 退化为可选快照备份。
...
| 真实评估 | 采样批次 | `state/results.json`（★R 矩阵，唯一真相） | 派生 Elo、预测器 |
| Elo 派生 | R 矩阵按模型列回放 | `state/elo_cache.json`（派生缓存，按模型列指纹失效） | 采样器、看板、报告 |
...
> 旧版单文件 `cluster_artifacts.pkl` 已按写者拆分为 `feature_cache.pkl`（特征缓存）+ `cluster_result.pkl`（聚类产物），避免双写者不同 schema 互覆盖。旧版 `state.json` 作为 Elo 主存储的角色已让位给 `results.json`（R 矩阵），Elo 由 `derive_elo(R, model)` 纯函数派生（`evaluation/elo_access.py` 统一入口）。
