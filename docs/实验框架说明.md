...
断点续跑：统一库的 trials 表是真相源（P4 起 db 唯一写入，旧世代 trials.jsonl 仅一次性导入）。重启 study 时，已成功完成的 (config × seed) 自动跳过；`bayesian` 还会把已有结果喂回 TPE 重建研究状态。
