...
一个系统化的黑盒 LLM 安全评估框架：自动生成攻击集 → 自适应攻击测试 → ELO 威胁排名 → 过敏检测 → 聚类分析 → 生成可读的 Markdown 安全报告。
...
新攻击方法不用从零预热。系统按文本/技术/意图/防御交互特征把它分到最近簇，用簇内已测方法的距离倒数加权 ELO 作为初始值。同一攻击基底的变体（如 `*_rot13`、`*_b64`）会优先相互借用 ELO。
...
防御方 ELO 收敛必须同时满足：轮次 Elo 标准差小、相对标准差小、已测覆盖率足够。避免“一轮全失败后因更新幅度小而误判收敛”。
...
- **L1**：纯文本提示工程（编码绕过、角色扮演、语义框架等），AI 直接生成
- **L2**：LLM 驱动攻击、对抗性自然语言后缀、多轮对话简化版，用生成模型模拟攻击 LLM 的构造过程

L2 攻击显著扩大攻击面，提供更多高质量攻击方法，缓解“优质攻击不足”导致的 ELO 低估问题。
...
1. **攻击生成**：`python -m llmsec.attacks.generate --level l1` 从 `llmsec/攻击分析.md` 解析 L1 方法，生成带数学题的攻击 prompt；`--level l2` 生成 L2 级攻击（LLM 驱动攻击、对抗性自然语言后缀、多轮对话简化版）。
2. **自适应攻击**：`runner` 用 ELO 驱动逐轮攻击。种子阶段每个预簇随机测 1 个方法建立 ground truth；后续轮次用采样器选择下一批方法，并根据 ELO 波动自适应调整 batch_size，直到收敛或达到最大轮次。
...
- **状态**：`output/state/state.json`（ELO + ground truth + 聚类锚点）
...
├── evaluation/   # evaluator(全量评估) / judge(LLM评分) / elo(反向ELO) / safe_twin(过敏检测)
...
```bash
python -m llmsec.attacks.generate [--level {l1,l2,all}] [--dry-run] [--only ID] [--output PATH]
    # 解析 llmsec/攻击分析.md 中的攻击方法
    # --level l1: 仅 L1 纯文本提示工程（默认）
    # --level l2: 仅 L2 LLM驱动/对抗性后缀/多轮简化攻击
    # --level all: L1 + L2
...
├── state/                  # 持久化状态
│   ├── state.json          #   统一 ELO + ground truth 状态
