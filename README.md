...
防御方 ELO 收敛必须同时满足：轮次 Elo 标准差小、相对标准差小、最近被测方法成功率接近 50%、已测覆盖率足够。避免“一轮全失败后因更新幅度小而误判收敛”。
...
2. **自适应攻击**：`runner` 用 ELO 驱动逐轮攻击。种子阶段每个预簇随机测 1 个方法建立 ground truth；后续轮次用采样器选择下一批方法，直到收敛或达到最大轮次。
...
python -m llmsec.attacks.generate [--dry-run] [--only 1.1.1] [--output PATH]
    # 解析 llmsec/攻击分析.md 中的 L1 方法，生成攻击集
