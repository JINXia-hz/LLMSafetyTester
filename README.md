...
```bash
# Step 1a: 生成我们自己的 L1 攻击集（一次性，约215条）
python generate_attacks.py

# Step 1b: 导入 HarmBench 公开数据集（300条）
python import_harmbench.py

# Step 2: 对目标模型执行评估（默认启用Judge + ELO）
python evaluate.py --max-samples 20                     # 快速测试自己的攻击集
python evaluate.py --input harmbench_prompts.jsonl      # 测试HarmBench数据集
...
# Step 5: 查看结果
# output/攻击集_L1_汇总.json       — 我们攻击集的统计
# output/harmbench_prompts_汇总.json — HarmBench数据集的统计
# output/elo.json                  — ELO排名（跨数据集合并）
...
| `output/elo.json` | JSON | ELO评分持久化状态（跨数据集） |
