...
- `HDBSCAN(min_cluster_size=max(3, n//40), min_samples=1, cluster_selection_method='eom')` 直接作用于白化坐标。`min_samples=1` 放宽互达距离，是集中空间里把噪声率从 ~76% 降到 ~67% 的关键参数（6 组参数实测对比）。
- **聚类树 = `single_linkage_tree_.to_numpy()`**：scipy 兼容 linkage (n-1, 4)，`fcluster` 任意层可切——树图缩放、层切割 API 全部通用。
...
- **命名**（`pipeline.py: auto_name_clusters`）：簇内最多技术标签 → 最多 harm_type → TF-IDF 关键词（剔除 base64/rot13 乱码 token）→ 小簇/单点簇用代表方法名兜底（保证非默认名）；噪声组固定命名"稀疏区（低密度噪声）"。
...
1. **TF-IDF 降级是当前最大瓶颈**：语义区分度弱 → 聚类噪声率高（6 组 HDBSCAN 参数实测 67% 已是最好）、弱监督信号微弱、ANOVA 不显著。有网环境 sentence-transformers 上线后这三项应同时改善。
...
1. 恢复在线 embedding（HF 可达或自建镜像）——预期同时改善聚类噪声率与 ANOVA 显著性；
2. 若 ANOVA 在在线 embedding 下仍不显著，则是特征抽象本身的答案：把反应相关信号（judge 维度、响应行为）做成半监督 embedding 目标，或引入对比学习训练"反应感知"的文本编码器；
